...
import { Sparkles, ListChecks, AlertTriangle, BookOpen } from "lucide-react";
...
function SessionView({ session }: { session: SessionDetail }) {
  ...
  return (
    <>
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="text-[10px] uppercase tracking-[2px] text-text-dim mb-1">
              {session.repo.owner}/{session.repo.name} · session #{session.idx}
              {" · "}
              {session.checkpointId.slice(0, 12)}
            </div>
            <SheetTitle className="text-[20px] leading-[1.3] pr-8">
              {session.summary?.intent && (
                <Sparkles
                  size={14}
                  className="inline-block text-green mr-1.5 -translate-y-[2px]"
                  aria-label="auto-summary intent"
                />
              )}
      ...
      {/* Auto-summary block (from entire-cli's summarize feature) */}
      {session.summary && <AutoSummaryPanel summary={session.summary} />}
...
function AutoSummaryPanel({ summary }: { summary: SessionSummaryBlock }) {
  const hasAny =
    !!summary.intent ||
    !!summary.outcome ||
    !!summary.friction?.length ||
    !!summary.open_items?.length ||
    !!summary.learnings?.repo?.length ||
    !!summary.learnings?.workflow?.length ||
    !!summary.learnings?.code?.length;
  if (!hasAny) return null;

  return (
    <section className="px-5 py-4 border-b border-border-main bg-[rgba(0,255,65,0.03)]">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-[10px] text-text-dim font-mono">
          generated by entire-cli at commit time
        </span>
      </div>
      ...
      <div className="space-y-3">
        {summary.intent && (
          <div>
            <div className="text-[10px] uppercase tracking-[1.5px] text-text-dim mb-1">
              Intent
            </div>
        ...
        {summary.friction && summary.friction.length > 0 && (
          <SummaryList
            icon={<AlertTriangle size={11} className="text-amber-warn" />}
            label="Friction"
            items={summary.friction}
            itemClass="text-amber-warn"
          />
        )}

        {summary.open_items && summary.open_items.length > 0 && (
          <SummaryList
            icon={<ListChecks size={11} className="text-[#58a6ff]" />}
            label="Open items"
        ...
        {(summary.learnings?.repo?.length ||
          summary.learnings?.workflow?.length) && (
          <div>
            <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-[1.5px] text-text-dim mb-1">
              <BookOpen size={11} className="text-green" /> Learnings
            </div>
            {summary.learnings?.repo && summary.learnings.repo.length > 0 && (
              <div className="mb-1.5">
                <div className="text-[10px] text-text-dim font-mono mb-0.5">
                  repo
                </div>
                <ul className="space-y-0.5">
                  {summary.learnings.repo.map((l, i) => (
                    <li
                      key={i}
                      className="text-[12px] text-text-secondary pl-3 border-l border-border-green"
                    >
                      {l}
                    </li>
                  ))}
            ...
            {summary.learnings?.workflow &&
              summary.learnings.workflow.length > 0 && (
                <div>
                  <div className="text-[10px] text-text-dim font-mono mb-0.5">
                    workflow
                  </div>
                  <ul className="space-y-0.5">
                    {summary.learnings.workflow.map((l, i) => (
                      <li
                        key={i}
                        className="text-[12px] text-text-secondary pl-3 border-l border-border-green"
                      >
        ...
        {summary.learnings?.code && summary.learnings.code.length > 0 && (
          <div>
            <div className="text-[10px] uppercase tracking-[1.5px] text-text-dim mb-1">
              Code findings
            </div>
            <ul className="space-y-1">
              {summary.learnings.code.map((c, i) => (
                <li
                  key={i}
                  className="text-[12px] text-text-secondary pl-3 border-l border-[#58a6ff]"
                >
                  {c.path && (
                  ...
                  {c.finding && <span className="ml-1">{c.finding}</span>}
                </li>
              ))}
            </ul>
          </div>
